...
ServerEvents.recipes((event) => {
    ...
    Ingredient.of('#productivebees:advanced_beehives').itemIds.forEach(
        (hive) => {
            if (hive.indexOf('canvas') != -1) return

            var strippedWood = hive
                .replace('productivebees', 'chipped')
                .replace('advanced', 'stripped')
                .replace('beehive', 'log')
            event.shaped(hive, ['AAA', 'EBE', 'DCD'], {
                A: `#${strippedWood}`,
                B: '#gtceu:circuits/lv',
