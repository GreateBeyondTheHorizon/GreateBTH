...
BlockEvents.rightClicked("minecraft:crafting_table", (event) => {
  if (!event.getPlayer().stages.has("early_game_completed")) {
    event.getPlayer()
      .displayClientMessage(Component.literal("Complete the Introduction quest chapter to be able to use this!")
        .red(), true);
    event.cancel();
  }
});
...
BlockEvents.rightClicked((event) => {
  if (event.block.tags.contains(new ResourceLocation("beds")) && !event.getPlayer().stages.has("crafted_bed")) {
    event.getPlayer()
      .displayClientMessage(Component.literal('Complete the "On the Floor No-More" quest to be able to use this!')
        .red(), true);
    event.cancel();
  }
});
...
BlockEvents.rightClicked("minecraft:dirt", (event) => {
  ...
  let holdingCeramicWaterBucket = event.item.id === "ceramicbucket:ceramic_bucket" && event.item.nbt?.Fluid.FluidName === "minecraft:water";
  if (!event.player.shiftKeyDown && (holdingWaterBucket || holdingCeramicWaterBucket)) {
    event.block.set("minecraft:mud");
    event.server.runCommandSilent(
      `execute in ${event.level.dimension}
      run execute as ${event.player.getUuid()}
      run playsound minecraft:entity.generic.splash ambient @a ${event.block.pos.x} ${event.block.pos.y} ${event.block.pos.z} 1 1`
    );
    event.player.swing();
