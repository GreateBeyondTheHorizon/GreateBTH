...
BlockEvents.rightClicked('minecraft:dirt', event => {
	...
	if (!event.player.shiftKeyDown && (holdingWaterBucket || holdingCeramicWaterBucket)) {
		...
		event.server.runCommandSilent(`execute in ${event.level.dimension} run execute as ${event.player.getUuid()} run playsound minecraft:entity.generic.splash ambient @a ${event.block.pos.x} ${event.block.pos.y} ${event.block.pos.z} 1 1`)
		event.player.swing()
