GTCEuStartupEvents.registry('gtceu:machine', event => {
    ...
    //multi
    event.create('ore_decimator', 'multiblock')
    .rotationState(RotationState.NON_Y_AXIS)
    .recipeType('ore_decimator')
    .appearanceBlock(GTBlocks.CASING_STAINLESS_CLEAN)
    .pattern(definition => FactoryBlockPattern.start()
        .aisle("PPPPP", "PTTTP", "PTTTP", "PPPPP")
        .aisle("PPPPP", "S#G#S", "S#G#S", "PPPPP")
        .aisle("PPPPP", "SWGWS", "SWGWS", "PPMPP")
        .aisle("PPPPP", "S#G#S", "S#G#S", "PPPPP")
        .aisle("PPPPP", "PTCTP", "PTTTP", "PPPPP")
        .where('C', Predicates.controller(Predicates.blocks(definition.get())))
        .where('P', Predicates.blocks('gtceu:inert_machine_casing').setMinGlobalLimited(5).or(Predicates.autoAbilities(definition.getRecipeType())))
...
GTCEuStartupEvents.registry('gtceu:recipe_type', event => {
    ...
    event.create('ore_decimator')
        .category('multi') //Used in the GTlib / LDlib ui editor
        .setEUIO('in')
        .setMaxIOSize(1, 1, 1, 0) //Max Item Inputs, Max Item Outputs, Max Fluid Inputs, Max Fluid Outputs
